...
def array(ctype, length, val=None):
    ...
    if str(ctype)[0] == '<':
        ...
        if ctype.split(" ")[1] == 'char_p' or ctype.split(" ")[1] == 'cstring':
            ...
            ctype = ctypes.c_char
    arr_type = ctype * length  # Create the array type
    if val is None:
        return arr_type()  # Default-initialized array
    else:
        return arr_type(*([val] * length))  # Initialize all elements to val
...
def array2d(ctype, rows, cols, val=None):
    ...
    if str(ctype)[0] == '<':
        ...
        if ctype.split(" ")[1] == 'char_p' or ctype.split(" ")[1] == 'cstring':
            ...
            ctype = ctypes.c_char
    row_type = ctype * cols  # Define the row type
    arr_type = row_type * rows  # Define the 2D array type
    if val is None:
        return arr_type()  # Default-initialized 2D array
    else:
        return arr_type(*([row_type(*([val] * cols)) for _ in range(rows)]))  # Initialize all elements to val
...
def array3d(ctype, depth, rows, cols, val=None):
    ...
    if str(ctype)[0] == '<':
        ...
        if ctype.split(" ")[1] == 'char_p' or ctype.split(" ")[1] == 'cstring':
            ...
            ctype = ctypes.c_char
    col_type = ctype * cols  # Define the column type
    row_type = col_type * rows  # Define the row type
    arr_type = row_type * depth  # Define the 3D array type
    if val is None:
        return arr_type()  # Default-initialized 3D array
    else:
